...
for (j in (1:length(dire.names))) {
  ...
  for (i in (1:(length(au.quaterspoints)-1))) {
    #i=1
    au.volume.mean[i]<-mean(na.omit(au.price[c((au.quaterspoints[i]+1):au.quaterspoints[i+1]),6]))
    au.amount.mean[i]<-mean(na.omit(au.price[c((au.quaterspoints[i]+1):au.quaterspoints[i+1]),7]))
  ...
  colnames(au.result)<-c("volatility","voi","oni","il1","il2","il3","volume mean","amount mean")
  #View(au.result)
  write.xlsx2(au.result, paste0('C:\\UM\\R\\calGARCH\\output\\futures voo\\',au.file,'/v',au.file,'.xls'),sheetName = "volatility, voi and oni", col.names = TRUE, row.names = TRUE)
