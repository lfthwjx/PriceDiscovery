...
cpi_month<-read.xlsx("D:\\UM\\calGARCH\\macro uncertainty 2nd\\bank monthly 96.04-16.06.xls", sheetIndex=1,startRow = 4, endRow = 247)
...
Box.test(ptm.cpi,lag=12,type = 'Ljung-Box')#p-value = 0.151
Box.test(ptm.cpi^2,lag=12,type = 'Ljung-Box')#ÐèÒª¶þ½×p-value = 5.32e-06
#?Box.test
ArchTest(x=ptm.cpi^2,lags=12)
...
vol_1.ts<-ts(vol_1,frequency = 12,start = c(1996,4))
...
#View(vol_out[,1])
vol_out_tmp<-t(as.matrix(vol_out[c(1:240),1]))
...
vol_season.ts<-ts(vol_season_mean,frequency = 4,start = c(1996,2))
#str(vol_season.ts)
#View(vol_season.ts)
vol_season.ts<-as.timeSeries(vol_season.ts)
#View(vol_season.ts)
write.xlsx(vol_season.ts, "D:\\UM\\calGARCH\\output\\bank.xlsx",sheetName = "CPI voltality seasonly",append=TRUE)